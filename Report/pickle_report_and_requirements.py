...
lst_verified = (arr.transpose()[4]).tolist()
...
for i in range(len(unique_tags)):
    for j in range(len(lst_tags)):
        if lst_tags[j] == unique_tags[i]:
            ...
            unique_verified.append(lst_verified[j])
            break
        
unique_tags = [i.replace(" ","") for i in unique_tags[:-1]]
unique_descriptions=[i.replace("\n"," ") for i in unique_descriptions[:-1]]

print(len(unique_tags),len(unique_descriptions), len(unique_descriptions))

for i,j,k in zip(unique_tags,unique_descriptions,unique_verified):
    print(i,'\t',j,'\t',k)
    print('-----------------------')

# create dictionary from requrements and save it
req_dict={ unique_tags[i] : [0,[],[],unique_descriptions[i],unique_verified[i]] for i in range(len(unique_tags)) }
print(req_dict)
